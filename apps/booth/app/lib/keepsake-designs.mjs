// Original stationery compositions. The controls, storage and delivery flows are unchanged.
...
function ornateFrame(color){let a=rect(48,48,1104,1704,'none',color,2)+rect(63,63,1074,1674,'none',color,.8);for(const [x,y,sx,sy]of [[77,78,1,1],[1123,78,-1,1],[77,1722,1,-1],[1123,1722,-1,-1]])a+=`<g transform="translate(${x} ${y}) scale(${sx} ${sy})">${flourish(0,0,1,color)}</g>`;return a;}
function laurel(x,y,s,color){return `<g transform="translate(${x} ${y}) scale(${s})">${path('M0 78C-100 65-100-37-44-75M0 78C100 65 100-37 44-75',color,2)}${[-1,1].map(side=>Array.from({length:8},(_,i)=>{const t=i/7*Math.PI*.86+Math.PI*.10,xx=side*Math.sin(t)*67,yy=70-Math.cos(t)*135;return `<path d="M ${xx} ${yy} q ${side*26} -25 ${side*37} -11 q ${-side*10} 28 ${-side*37} 11" fill="${color}" opacity="${.6+i*.045}"/>`}).join('')).join('')}</g>`;}
...
export function renderKeepsake({photo='',cfg={},monogram='',template='ivory',filter='none',id='card'}={}){
 ...
 if(l==='botanical'){b={x:145,y:145,w:910,h:1080,rx:0};headY=1310;nameY=1505;nameSize=169;}
 ...
 if(l==='polaroid'){b={x:85,y:100,w:1030,h:1170,rx:0};headY=1340;nameY=1510;nameSize=168;}
 ...
 if(l==='deco')art+=ornateFrame(gold);
 ...
 if(l==='diploma')art+=rect(43,43,1114,1714,'none',gold,3)+rect(57,57,1086,1686,'none',d.accent,1)+laurel(600,1320,.45,d.accent);
 ...
 if(l==='spotlight')art+=path('M0 46H1200M50 1380H1150',gold,3);
 ...
 if(['botanical','deco','jewel'].includes(l))art+=flourish(460,1320,.26,d.accent)+flourish(740,1320,.26,d.accent,-1);
 art+='<g data-text-role="name">'+lettering(c.title,600,nameY,1010,nameSize,footerInk,{face:nameFace,lines:2,bounds:[1380,1615]})+'</g>';
 ...
 if(c.type==='birthday'&&c.seal){const ax=l==='ticket'?1035:l==='disco'?152:1040,ay=1363;art+=`<circle cx="${ax}" cy="${ay}" r="55" fill="${d.ink}" stroke="${gold}" stroke-width="3"/><circle cx="${ax}" cy="${ay}" r="46" fill="none" stroke="${d.paper}" stroke-width="1"/>`+lettering(c.seal,ax,ay+20,90,64,d.paper,{face:'serif'});}
