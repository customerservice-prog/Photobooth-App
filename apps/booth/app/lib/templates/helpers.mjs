...
export function svg(ctx,{photo,before='',after='',copy='',defs=''}={}){
 ...
 return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1800" width="1200" height="1800" role="img" aria-label="${esc(t.name+' — '+ctx.copy.title)}" data-collection="event-collections-v1" data-template-id="${t.id}" data-design="${t.eventType}-${t.legacyId}" data-layout="${t.layout}"><title>${esc(ctx.copy.title)}</title><defs>${goldDefs(id)}<clipPath id="${id}-photo">${shape}</clipPath>${defs}</defs>${rect(0,0,1200,1800,t.paper)}${before}<g clip-path="url(#${id}-photo)">${rect(x,y,w,h,t.photoMat||'#e6e1d5')}${picture}</g><g fill="none" stroke="${gold}" stroke-width="2">${shape.replace(/fill="[^"]*"/g,'fill="none"')}</g>${after}${copy}</svg>`;
}
export const gold=ctx=>`url(#${ctx.id}-foil)`;
export function nameBlock(ctx,{value=ctx.copy.title,y=1500,width=980,size=160,face='script',ink=ctx.template.ink,bounds=[1380,1605]}={}){return `<g data-text-role="name">${text(value,600,y,width,size,ink,{face,lines:2,bounds})}</g>`;}
...
// Deterministic, stationary paper embellishment. All fragments are confined to margins.
