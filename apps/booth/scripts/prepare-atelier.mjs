...
const version='atelier-assets-2',file='app/lib/generated/atelier-assets.mjs',out='proof-output';
...
const hashes={};async function get(url){let last;for(let n=0;n<3;n++){try{const r=await fetch(url,{signal:AbortSignal.timeout(45000)});if(!r.ok)throw new Error(`Asset fetch ${r.status}`);const bytes=Buffer.from(await r.arrayBuffer());hashes[url]=createHash('sha256').update(bytes).digest('hex');return bytes;}catch(e){last=e;if(n<2)await new Promise(r=>setTimeout(r,1000));}}throw last;}
...
// Remove detached paper specks, not the connected painted flower/leaves.
...
for(const [name,folder,filename]of [['script','greatvibes','GreatVibes-Regular.ttf'],['serif','cormorantgaramond','CormorantGaramond[wght].ttf'],['sans','montserrat','Montserrat[wght].ttf']]){
 const root=`https://raw.githubusercontent.com/google/fonts/main/ofl/${folder}/`;const bytes=await get(root+encodeURIComponent(filename)),font=opentype.parse(bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength));
