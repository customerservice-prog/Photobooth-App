...
test('wedding and birthday approved artwork is pixel-source-preserved',()=>{for(const event of ['wedding','birthday'])for(const t of getTemplatesForEvent(event)){const args={cfg:cfg(event),id:'same',template:t.legacyId};const rendered=renderKeepsake(args).replace(/data-template-id="[^"]*" /,'');assert.equal(rendered,oldRender(args));}});
...
test('whole-photo mode does not clip corners on arched and cutout cards',()=>{const s=renderKeepsake({cfg:{type:'mitzvah',photoFit:'fit'},template:'mitzvah_mosaic',photo:'data:image/jpeg;base64,/9j/2Q=='});assert(s.includes('xMidYMid meet'));assert(s.includes('<clipPath id="card-photo"><rect'));});
