...
for(const type of types){
 ...
 test(type+' fit mode and photo finish propagate into every composition',()=>{for(const d of getDesigns(type)){const s=renderKeepsake({photo,cfg:{type,photoFit:'fit'},template:d.id,filter:'grayscale(1) contrast(1.08) brightness(1.04)'});assert(s.includes('xMidYMid meet'));assert(s.includes('filter:grayscale(1)'));}});
